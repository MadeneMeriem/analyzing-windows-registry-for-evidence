...
Introduction
In windows there is an interesting concept which is Windows Registry , a critical hierarchical database that contains low-level settings for the OS and the apps installed. Most of the malwares and malicious processes often modify or create registry keys for persistence and detection evasion, thus analysing them can reveal traces and evidence of such bad activities and provide valuable information during the forensics and investigations workflow. For this project , we are interested in using volatility to analyse and dig through Windows registry hives extracted from a memory dump of an infected machine to identify security incidents . 
Lab Setup
...
We can see a key written by a suspicious process called venom.exe, running from C:\Users\yamina\AppData\Local\Temp\venom.exe. This is clearly not a standard Windows process, and as we can see it is being executed from the temp folder which indicates a strong and standard malicious activity . 
...
Those entries were written by cmd.exe at the same time the malware was being executed. It set them to delete the cached Onedrive update binaries and remove the installation folder , this might might be clean up and cover to the traces 
...
13:22:10
                            7z1900-x64.exe executed
13:24:08
                            UserAccountControlSettings.exe executed
13:30:43
                             Command Prompt opened
13:37:43
ed0a26904442535518a83879b77cee6e89eea4fa18521be4308d49a6fa290c58.exe executed
13:34 - 13:38
                             Edge, Explorer, Control Panel activity
